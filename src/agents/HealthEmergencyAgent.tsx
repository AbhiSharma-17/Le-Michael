...
export default function HealthEmergencyAgent({ onBack }: { onBack: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({ messages: newMessages, agent: "health" }),
      });
      ...
      // Formatting JSON response to be more readable if it returns JSON
      let content = data.text;
      try {
        if (content.startsWith("{") || content.includes('```json')) {
          const rawJson = content.replace(/```json/g, '').replace(/```/g, '').trim();
          const parsed = JSON.parse(rawJson);
          content = `**Emergency Type:** ${parsed.emergency_type || 'Unknown'}\n**Severity:** ${parsed.severity || 'Unknown'}\n\n**Symptoms:**\n${(parsed.symptoms || []).map((s: string) => `- ${s}`).join('\\n')}\n\n**Recommended First Aid:**\n${(parsed.recommended_first_aid || []).map((s: string) => `- ${s}`).join('\\n')}\n\n**Questions:**\n${(parsed.questions || []).map((s: string) => `- ${s}`).join('\\n')}\n\n${parsed.call_emergency_services ? '🚨 **Please contact emergency services immediately.**' : ''}`;
        }
      } catch (e) {
  ...
  return (
    <div className="flex flex-col h-dvh bg-gray-50 font-sans">
      ...
      <header className="flex items-center gap-3 bg-rose-600 text-white p-4 shadow-md shrink-0">
        ...
        <HeartPulse size={24} />
        <div>
          <h1 className="text-lg font-bold tracking-wide leading-tight">Health Emergency AI</h1>
          <p className="text-xs text-rose-200 font-medium">Immediate medical guidance</p>
        </div>
      </header>
        ...
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-gray-500 space-y-4">
            ...
            <p className="text-center max-w-sm font-medium">
              I am the Health Emergency AI. Describe the medical emergency, and I will provide immediate first-aid guidance.
            </p>
            <div className="grid grid-cols-1 gap-2 mt-4 w-full max-w-md">
              ...
              <button 
                onClick={() => setInput("My child swallowed some cleaning liquid.")}
                className="bg-white border border-gray-200 p-3 rounded-lg text-sm text-left hover:bg-gray-50 transition-colors"
              >
                "My child swallowed some cleaning liquid."
              </button>
            </div>
          </div>
        )}
