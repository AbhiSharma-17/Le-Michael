...
export default function SearchRescueAgent({ onBack }: { onBack: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({ messages: newMessages, agent: "rescue" }),
      });
  ...
  return (
    <div className="flex flex-col h-dvh bg-gray-50 font-sans">
      {/* Header */}
      <header className="flex items-center gap-3 bg-amber-600 text-white p-4 shadow-md shrink-0">
        <button onClick={onBack} className="mr-2 hover:bg-amber-700 p-2 rounded-full transition-colors">
          &larr; Back
        </button>
        <Radar size={24} />
        <div>
          <h1 className="text-lg font-bold tracking-wide leading-tight">Search & Rescue AI</h1>
          <p className="text-xs text-amber-200 font-medium">Assisting trapped or missing persons</p>
        </div>
      </header>
        ...
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-gray-500 space-y-4">
            ...
            <p className="text-center max-w-sm font-medium">
              I am the Search & Rescue AI. I can guide you on how to stay safe if trapped, and help emergency responders locate you.
            </p>
            <div className="grid grid-cols-1 gap-2 mt-4 w-full max-w-md">
              <button 
                onClick={() => setInput("My family is trapped after the earthquake.")}
              ...
              <button 
                onClick={() => setInput("We are trapped by flood water.")}
                className="bg-white border border-gray-200 p-3 rounded-lg text-sm text-left hover:bg-gray-50 transition-colors"
              >
