  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({ messages: newMessages, agent: "voice" }),
      });
      ...
      try {
        if (content.startsWith("{") || content.includes("\`\`\`json")) {
          ...
          content = `**Language:** ${parsed.language || "Unknown"}\n**Emotion:** ${parsed.emotion || "Unknown"}\n**Panic Level:** ${parsed.panic_level || "Unknown"}\n\n**Summary:** ${parsed.summary || "None"}\n\n**Important Information:**\n${(parsed.important_information || []).map((s: string) => "- " + s).join("\\n")}\n\n**Next Questions:**\n${(parsed.next_questions || []).map((s: string) => "- " + s).join("\\n")}\n\n**Recommended Agent:** ${parsed.recommended_agent || "Guardian Agent"}`;
        }
      } catch (e) {
  ...
  return (
    <div className="flex flex-col h-dvh bg-gray-50 font-sans">
      ...
      <header className="flex items-center gap-3 bg-purple-600 text-white p-4 shadow-md shrink-0">
        <button
          onClick={onBack}
          className="mr-2 hover:bg-purple-700 p-2 rounded-full transition-colors"
        >
          &larr; Back
        </button>
        <Mic size={24} />
        ...
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-gray-500 space-y-4">
            ...
            <p className="text-center max-w-sm font-medium">
              I am the Voice & Conversation AI. I am here to listen, calm you
              down, and collect initial emergency information.
            </p>
            <div className="grid grid-cols-1 gap-2 mt-4 w-full max-w-md">
              <button
                onClick={() => setInput("My father collapsed!")}
                className="bg-white border border-gray-200 p-3 rounded-lg text-sm text-left hover:bg-gray-50 transition-colors"
              >
                "My father collapsed!"
              </button>
              <button
                onClick={() => setInput("There is a fire.")}
                className="bg-white border border-gray-200 p-3 rounded-lg text-sm text-left hover:bg-gray-50 transition-colors"
              >
                "There is a fire."
              </button>
              <button
                onClick={() => setInput("Someone is following me.")}
                className="bg-white border border-gray-200 p-3 rounded-lg text-sm text-left hover:bg-gray-50 transition-colors"
              >
