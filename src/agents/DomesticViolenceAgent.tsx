...
export default function DomesticViolenceAgent({ onBack }: { onBack: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({ messages: newMessages, agent: "domestic" }),
      });
  ...
  return (
    <div className="flex flex-col h-dvh bg-gray-50 font-sans">
      {/* Header */}
      <header className="flex items-center gap-3 bg-pink-600 text-white p-4 shadow-md shrink-0">
        <button onClick={onBack} className="mr-2 hover:bg-pink-700 p-2 rounded-full transition-colors">
          &larr; Back
        </button>
        <Home size={24} />
        <div>
          <h1 className="text-lg font-bold tracking-wide leading-tight">Domestic Violence AI</h1>
          <p className="text-xs text-pink-200 font-medium">Safe & supportive guidance</p>
        </div>
      </header>
        ...
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-gray-500 space-y-4">
            ...
            <p className="text-center max-w-sm font-medium">
              I am the Domestic Violence AI. I provide a safe, non-judgmental space to help you stay safe and create a safety plan.
            </p>
            <div className="grid grid-cols-1 gap-2 mt-4 w-full max-w-md">
              <button 
                onClick={() => setInput("My partner keeps threatening me.")}
              ...
              <button 
                onClick={() => setInput("I'm afraid to go home.")}
                className="bg-white border border-gray-200 p-3 rounded-lg text-sm text-left hover:bg-gray-50 transition-colors"
              >
