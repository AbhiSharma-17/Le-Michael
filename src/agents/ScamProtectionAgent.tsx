...
export default function ScamProtectionAgent({ onBack }: { onBack: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({ messages: newMessages, agent: "scam" }),
      });
  ...
  return (
    <div className="flex flex-col h-dvh bg-gray-50 font-sans">
      {/* Header */}
      <header className="flex items-center gap-3 bg-emerald-600 text-white p-4 shadow-md shrink-0">
        <button onClick={onBack} className="mr-2 hover:bg-emerald-700 p-2 rounded-full transition-colors">
          &larr; Back
        </button>
        <ShieldCheck size={24} />
        <div>
          <h1 className="text-lg font-bold tracking-wide leading-tight">Scam Protection AI</h1>
          <p className="text-xs text-emerald-200 font-medium">Protecting money & identity</p>
        </div>
      </header>
        ...
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-gray-500 space-y-4">
            ...
            <p className="text-center max-w-sm font-medium">
              I am the Scam Protection AI. I can help you identify scams, secure your accounts, and protect your identity.
            </p>
            <div className="grid grid-cols-1 gap-2 mt-4 w-full max-w-md">
              <button 
                onClick={() => setInput("I scanned a QR code and ₹20,000 disappeared.")}
              ...
              <button 
                onClick={() => setInput("I received a call asking for my OTP.")}
                className="bg-white border border-gray-200 p-3 rounded-lg text-sm text-left hover:bg-gray-50 transition-colors"
              >
