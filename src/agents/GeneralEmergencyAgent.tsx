...
export default function GeneralEmergencyAgent({ onBack }: { onBack: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/chat", {
        ...
        body: JSON.stringify({ messages: newMessages, agent: "general" }),
      });
  ...
  return (
    <div className="flex flex-col h-dvh bg-gray-50 font-sans">
      ...
      <header className="flex items-center gap-3 bg-teal-600 text-white p-4 shadow-md shrink-0">
        <button onClick={onBack} className="mr-2 hover:bg-teal-700 p-2 rounded-full transition-colors">
          &larr; Back
        </button>
        <LifeBuoy size={24} />
        <div>
          <h1 className="text-lg font-bold tracking-wide leading-tight">General Emergency Agent</h1>
          <p className="text-xs text-teal-200 font-medium">Ready to assist safely</p>
        </div>
      </header>
        ...
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-gray-500 space-y-4">
            ...
            <p className="text-center max-w-sm font-medium">
              I am the General Emergency AI. Describe your situation, and I will help provide safe, immediate guidance.
            </p>
            <div className="grid grid-cols-1 gap-2 mt-4 w-full max-w-md">
              <button 
                onClick={() => setInput("My child accidentally swallowed some cleaning liquid.")}
                className="bg-white border border-gray-200 p-3 rounded-lg text-sm text-left hover:bg-gray-50 transition-colors"
              >
                "My child accidentally swallowed some cleaning liquid."
              </button>
              <button 
                onClick={() => setInput("Someone just got a severe electrical shock and is on the ground.")}
                className="bg-white border border-gray-200 p-3 rounded-lg text-sm text-left hover:bg-gray-50 transition-colors"
              >
                "Someone just got a severe electrical shock and is on the ground."
              </button>
            </div>
          </div>
        )}
